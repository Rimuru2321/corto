document.addEventListener('DOMContentLoaded', () => {
  ...
  if (voters.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4"><div class="empty-state">Aún no hay votos registrados en esta sesión.</div></td></tr>`;
  } else {
    ...
    tbody.innerHTML = sortedVoters.map((v, index) => {
      ...
      let voteHtml = '—';
      const votedId = v.courseId;
      
      if(votedId) {
        const course = courses.find(c => c.id === votedId);
        if(course) {
          voteHtml = `<div class="course-badge" style="background:${course.bg}; color:${course.fg}; border: 1px solid ${course.c1}33;">
      ...
      return `
        <tr>
          ...
          <td><div style="font-weight:700; color:var(--g900); display:flex; align-items:center; gap:0.5rem;"><span style="background:var(--g100); padding:5px; border-radius:50%; font-size:12px;">🕵️</span> Votante Anónimo</div></td>
  ...
  const dataVotes = courses.map(c => c.votes);
